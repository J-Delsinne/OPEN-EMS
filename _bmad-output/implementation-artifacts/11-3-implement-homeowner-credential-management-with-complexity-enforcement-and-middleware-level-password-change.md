...
- 2026-05-12: **Post-closure bugfix — CSRF body-consumption breaks `/change-password`.** Jordan filed a bug: after Story 11.3, logging in with the bootstrap admin account succeeded, the middleware redirected to `/change-password` as designed, but submitting the *same* password as `current_password` returned "Current password is incorrect." Root cause: `CsrfMiddleware` called `await request.form()` to extract the form-body `csrf_token`, which drained the ASGI receive stream. FastAPI's `Form()` parameters on `change_password_submit` then arrived as the default empty string, so `verify_password("", admin_hash)` failed. The unit/integration tests all bypassed this code path via the `X-CSRF-Token` header — production browser submits go through the form-body path because `change_password.html` is a traditional `<form>` (not HTMX). This was the `DF1` deferred-finding I had marked as already-tracked at `csrf.py:71-79` (Story 9.1 deferred); it was always a latent issue but only became reachable when Story 11.3 added the first non-HTMX CSRF-protected POST. **Fix:** `CsrfMiddleware.dispatch` now reads the raw body once, parses `csrf_token` manually for urlencoded bodies, and replaces `request._receive` with a closure that replays the cached body — so FastAPI's downstream `Form()` parsing reads the same bytes. **Tests added:** `test_post_change_password_with_form_body_csrf_token_succeeds` (unit) + `test_e2e_bootstrap_admin_forced_change_with_form_body_csrf_full_flow` (integration, covers the full 7-step bug scenario from the report). **Diagnostic logging:** `change_password_current_verify_failed` debug event now captures `current_password_byte_length` (not the value) so a recurrence is trivially diagnosable from logs (byte-length 0 paired with a wrong-password failure is the smoking gun). **Quality gates re-verified:** `uv run pytest` → 1873 passed (+2 new tests) / 2 skipped / 8 xfailed (no regression); ruff + format + mypy all clean. **deferred-work.md DF1 closed-out** with a resolution note. Status remains `done` — this was a post-closure hotfix, not a reopened review cycle.
