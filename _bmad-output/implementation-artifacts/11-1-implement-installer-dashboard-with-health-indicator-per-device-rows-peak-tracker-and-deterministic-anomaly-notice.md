...
Status: done
_Status set to done by bmad-code-review on 2026-05-12 after review-closure gate passed (5b): 26 resolved, 0 dismissed, 0 deferred-and-verified (the 3 Deferred-section bullets DF1/DF2/DF3 are checked-and-linked into deferred-work.md; the gate classified them as Resolved per the criterion-1 priority rule)._
...
11. **AC10 — All state reads come from StateStore + peak_intervals + event_log; ZERO additional DB queries for anomaly detection.**
    ...
    **Test #2 (route-level — measures actual DB load):** `test_installer_dashboard_render_db_query_count_bounded` instruments the aiosqlite connection (custom row factory + execute hook, OR a SQL-capture context manager) and asserts that GET `/installer/dashboard` (the shell route) triggers **zero data-surface DB queries** — i.e., zero queries against `peak_intervals`, `event_log`, or `device_registry` (the three data tables exposed by 11.1's surface). Session-auth queries against the `sessions` and `users` tables are exempt: they are framework-level (every authenticated request reads them via `require_installer`) and not specific to this story. The total query count is bounded (typically 2-3 for session + user reads) so the count-bound check is `counter.count < 10` plus the substring assertion against forbidden table names. _D3 resolution (P20 review fix, 2026-05-12): the original AC text "zero DB queries" was tightened to "zero data-surface DB queries" so the literal contract matches the actual implementation; the intent ("dashboard's data-fetch surface is template-only") was never in question._

    The fragment endpoints — when subsequently fetched by HTMX — trigger the documented data-surface counts (`auth queries excluded` as above):
    - `/fragments/installer/health-indicator` — 0 data-surface queries (snapshot read only)
    - `/fragments/installer/device-rows` — 1 data-surface query (DeviceRepo.list_all on `device_registry`)
    - `/fragments/installer/peak-tracker` — 1 data-surface query (EnergyRepo.get_current_monthly_peak_kw on `peak_intervals`) + 0 for constraints (in-memory provider)
    - `/fragments/installer/event-log-preview` — 1 data-surface query (EventLogRepo.list_recent on `event_log`)
    - `/fragments/installer/anomaly-notice` — 0 data-surface queries (snapshot + dismiss-dict only)
...
_Recorded by bmad-code-review on 2026-05-12. Adversarial review (Blind Hunter + Edge Case Hunter + Acceptance Auditor) produced 3 decision-needed / 17 patch / 3 defer / ~45 dismissed-as-noise. Jordan resolved all 3 decisions to option 1 on 2026-05-12; D1/D2/D3 are now patches (P18/P19/P20). 20 patches total await application before the A7 review-closure gate._

#### Decisions resolved (3 → all patched)

- [x] [Review][Decision→Patched] **D1 → P18 — Treat `ComponentState.stale` on a required role as `DEVICE_UNAVAILABLE` for anomaly purposes** ([src/open_ems/services/installer_anomaly.py:130]). Resolution: option 1 — count stale-on-required as DEVICE_UNAVAILABLE so the anomaly notice contract matches the device-row display contract (spec lines 85-86). See P18 below.
- [x] [Review][Decision→Patched] **D2 → P19 — Add a TTL/LRU bound to `_DISMISSED_ANOMALIES`** ([src/open_ems/services/installer_anomaly.py:267, 292-294]). Resolution: option 1 — bounded `collections.OrderedDict` with LRU eviction (max 100 entries). Avoids the session-lifecycle coupling of option 2 and the deployment-constraint hand-wave of option 3. See P19 below.
- [x] [Review][Decision→Patched] **D3 → P20 — Update AC10 #2 spec text to acknowledge auth queries are exempt** ([_bmad-output/implementation-artifacts/11-1-...md AC10 #2]). Resolution: option 1 — one-line spec edit clarifying "zero **data-surface** DB queries" (i.e., zero on `peak_intervals` / `event_log` / `device_registry`; session-auth queries are exempt). See P20 below.

#### Patches (17)

- [x] [Review][Patch] **P1 — HIGH — `db_query_counter` stores only the first SQL keyword, making the AC10 #2 forbidden-table assertion structurally tautological** [tests/utils/db_query_counter.py:45, tests/unit/web/test_installer_dashboard_route.py]. `counter.queries.append(sql.strip().split()[0].upper())` drops the table name; `assert "peak_intervals" not in " ".join(counter.queries)` cannot ever fire. Fix: store normalized full SQL; add a hard `expected_max_count` assertion.
- [x] [Review][Patch] **P2 — HIGH — Health-indicator fragment is missing the AC9 event-log link** [src/open_ems/web/templates/fragments/installer/health-indicator.html, src/open_ems/web/state_serialization.py:663-720]. AC9 lists three link surfaces; anomaly notice and per-device-row links are present, health-indicator link is absent. Fix: when `display != "NORMAL"`, emit `<a href="/installer/event-log?type=SYSTEM&window=24h">View in event log</a>` next to the badge; add `event_log_link` to builder context.
- [x] [Review][Patch] **P3 — MEDIUM — Cold-start summary override hides FAIL-level severity** [src/open_ems/services/installer_anomaly.py:179-180]. `if snapshot.sequence_id == 0: return "System is starting up"` runs unconditionally. A FAIL-severity badge with "starting up" summary is contradictory. Fix: gate override on `severity != "FAIL"` (or restrict to the SYSTEM_DEGRADED-only case).
- [x] [Review][Patch] **P4 — MEDIUM — Divide-by-zero in peak tracker when `peak_limit_kw == 0.0`** [src/open_ems/web/state_serialization.py:801]. `ratio = current_month_peak_kw / peak_limit_kw` has no guard. Fix: branch on `peak_limit_kw <= 0.0` early (treat as "not configured" or "exceeded"); add unit test.
- [x] [Review][Patch] **P5 — MEDIUM — CSRF token in `hx-headers` JSON attribute relies on accidental safety** [src/open_ems/web/templates/fragments/installer/anomaly-notice.html:24]. `hx-headers='{"X-CSRF-Token": "{{ csrf_token }}"}'` — Jinja autoescape rewrites `"`, `&`, `<`, `>`, `'`. If the token generator ever emits any of those, JSON breaks and Dismiss POSTs 403. Fix: `hx-headers='{{ {"X-CSRF-Token": csrf_token}|tojson|forceescape }}'`.
- [x] [Review][Patch] **P6 — MEDIUM — `device_id` is interpolated into URL without URL-encoding** [src/open_ems/web/state_serialization.py — `event_log_link` in device-row builder]. OCPP / Modbus / MQTT device IDs can contain `&`, `?`, `#`, spaces; the resulting link breaks Story 11.2's URL parser. Fix: `from urllib.parse import quote` and emit `quote(device_id, safe='')`. Test with a device_id containing `&`.
- [x] [Review][Patch] **P7 — MEDIUM — Per-fragment DB query-count tests are missing (AC10 #2)** [tests/unit/web/test_installer_dashboard_fragments.py]. AC10 #2 lists specific per-fragment counts (health=0, device-rows=1, peak-tracker=1, event-log-preview=1, anomaly-notice=0); only the shell test exists. Fix: add 5 tests, each wrapping the corresponding GET in `async with count_queries() as counter:` and asserting the documented count (most useful after P1 lands).
- [x] [Review][Patch] **P8 — LOW — `_ANOMALY_SUMMARIES` table promised by spec is absent** [src/open_ems/services/installer_anomaly.py:163-197]. Spec line 220: "The exact wording table lives as `_ANOMALY_SUMMARIES` in `installer_anomaly.py`. Single-source-of-truth — never inline anywhere else." The diff inlines the strings. Fix: extract into a module-level mapping; have `_build_anomaly_summary` look up parts via the table.
- [x] [Review][Patch] **P9 — LOW — POST dismiss bypasses the canonical dict lookup in its own render path** [src/open_ems/web/routes/actions.py:487]. The handler passes `current` (in-flight value) to the builder instead of `get_dismissed_signature(user.session_id)`. If `dismiss_signature` ever silently fails (future bounded-dict eviction), the POST response still derives "suppress" tautologically. Fix: `build_installer_anomaly_notice_context(snapshot, get_dismissed_signature(user.session_id))`.
- [x] [Review][Patch] **P10 — LOW — `_clear_dismiss_state` autouse fixture has no teardown** [tests/unit/services/test_installer_anomaly.py + copies in fragments + e2e]. Setup-only fixture leaves state across tests. Fix: convert to yield-style — `_reset_dismiss_state_for_tests(); yield; _reset_dismiss_state_for_tests()`.
- [x] [Review][Patch] **P11 — LOW — Bare `except Exception` in event-log preview swallows all programming errors** [src/open_ems/web/state_serialization.py:850]. Currently catches AttributeError / TypeError / any future bug as "missing zoneinfo data". Fix: narrow to `except (zoneinfo.ZoneInfoNotFoundError, ImportError):` and `logger.warning` first fallback.
- [x] [Review][Patch] **P12 — LOW — Health-indicator defensive fallback emits an unstyled CSS modifier class** [src/open_ems/web/state_serialization.py:663-720]. `display.lower()` for an unmapped enum yields class `health-indicator__badge--<unknown>` with no CSS rule. Fix: default css_modifier to `"degraded"` and `logger.warning` when fallback fires.
- [x] [Review][Patch] **P13 — LOW — Test name `test_anomaly_notice_returns_empty_section_when_normal` only checks "Dismiss" absent** [tests/unit/web/test_installer_dashboard_fragments.py]. A regression that emits a `<p>WARN summary</p>` without a Dismiss button passes the test. Fix: add `assert ">FAIL<" not in body and ">WARN<" not in body and "View in event log" not in body`.
- [x] [Review][Patch] **P14 — LOW — Missing AC14 #36 test for both UTC ISO and local timezone in event-log preview** [tests/unit/web/test_installer_dashboard_fragments.py]. AC12 / AC14 #36 require asserting BOTH `data-utc="..."` AND a `CEST`/`CET` substring appear in rendered HTML. Fix: add `test_event_log_preview_renders_utc_iso_in_data_utc_attribute_and_local_timezone_text`.
- [x] [Review][Patch] **P15 — LOW — Missing AC14 #44 GET-level test for re-render on new anomaly type after dismiss** [tests/unit/web/test_installer_dashboard_fragments.py]. The severity-elevation GET test exists; the new-type variant only exists at the pure-function level. Fix: add `test_get_fragment_after_dismiss_re_renders_on_new_anomaly_type` with `dismissed={DEVICE_ERROR}`, `current={DEVICE_ERROR, SYSTEM_DEGRADED}`.
- [x] [Review][Patch] **P16 — LOW — Missing structural test asserting at most one `.installer-anomaly-notice` element renders** [tests/unit/web/test_installer_dashboard_fragments.py]. AC12 explicit promise: "a structural test asserts that no second `.installer-anomaly-notice` element can render". Fix: GET the anomaly-notice fragment in FAIL state, assert `body.count('class="installer-anomaly-notice') == 1`.
- [x] [Review][Patch] **P17 — LOW — Fragile substring assertion in `test_event_log_preview_renders_5_most_recent`** [tests/unit/web/test_installer_dashboard_fragments.py]. `body.count("event-0") == 1` breaks if fixture range extends past 9 (`event-1` matches `event-1` AND `event-10..19`). Fix: change fixture summaries to non-overlapping unique markers (UUID or pinned-suffix).
- [x] [Review][Patch] **P18 — MEDIUM — Treat `ComponentState.stale` on required role as `DEVICE_UNAVAILABLE`** [src/open_ems/services/installer_anomaly.py:130]. Resolution of D1. Fix: extend the required-role check to `elif component_state in (ComponentState.unavailable, ComponentState.stale) and role in REQUIRED_DEVICE_ROLES:`. Add a unit test `test_detect_anomaly_required_role_stale_returns_fail_device_unavailable`. The device-row display contract (stale → UNAVAILABLE) and the anomaly contract now agree.
- [x] [Review][Patch] **P19 — MEDIUM — Add TTL/LRU bound to `_DISMISSED_ANOMALIES`** [src/open_ems/services/installer_anomaly.py:267, 270-294]. Resolution of D2. Fix: replace `_DISMISSED_ANOMALIES: dict[str, AnomalySignature] = {}` with `_DISMISSED_ANOMALIES: collections.OrderedDict[str, AnomalySignature] = collections.OrderedDict()`. Inside `dismiss_signature`, before writing, evict the oldest entry if `len(_DISMISSED_ANOMALIES) >= _MAX_DISMISS_ENTRIES` (constant set to 100). Use `_DISMISSED_ANOMALIES.move_to_end(session_id)` after each get/set to maintain LRU order. Add a unit test `test_dismiss_signature_evicts_oldest_entry_when_bound_reached`.
- [x] [Review][Patch] **P20 — LOW — Update AC10 #2 spec text to acknowledge auth queries are exempt** [this file, AC10 #2 description]. Resolution of D3. Fix: change "GET `/installer/dashboard` (the shell route) triggers **zero** DB queries (template-only)" to "GET `/installer/dashboard` (the shell route) triggers **zero data-surface DB queries** (no queries against `peak_intervals`, `event_log`, or `device_registry`; session-auth queries on the `sessions` table are exempt — they are framework-level, not story-specific)". Apply equivalently to AC12 if cross-referenced.

#### Deferred (3)

- [x] [Review][Defer] **DF1 — MEDIUM — `db_query_counter` only patches `Connection.execute`, missing `executemany`/`executescript`/`Cursor.execute`** [tests/utils/db_query_counter.py]. Future-proofing only — current production code uses `connection.execute` exclusively; the gap is not currently exploited. Deferred to a future test-infrastructure hardening sweep — see deferred-work.md.
- [x] [Review][Defer] **DF2 — LOW — Peak rounds to `0.0` falsely renders "No data yet"** [src/open_ems/web/state_serialization.py:775, 793]. A real peak of 0.04 kW rounds to 0.0 and shows "No data yet". Proper fix requires `EnergyRepo.get_current_monthly_peak_kw` signature change (return `None` for empty rows) — wider than 11.1's scope. Deferred — see deferred-work.md.
- [x] [Review][Defer] **DF3 — LOW — No negative CSRF test for `POST /actions/dismiss-anomaly`** [tests/unit/web/test_installer_dashboard_fragments.py]. CSRF is enforced globally by `CsrfMiddleware`; no negative test asserts rejection of CSRF-less requests. Same pattern as prior 10.x stories — deferred to a broader CSRF-coverage hardening sub-story. See deferred-work.md.

#### Dismissed (~45 — counts only)

Findings dismissed as noise included: speculative future enum members (future `SystemOperatingMode`, `DeviceRole`, `EventType` additions handled by exhaustiveness tests), defensive NaN/Inf/negative guards on values already constrained upstream, half-hour-offset DST formatting edge cases, dark-mode / RTL / `prefers-reduced-motion` CSS gaps (not in story scope), multi-process deployment concerns (single-worker deployment is the v1 contract), schema-migration column-count guards on a stable table, session_id reuse across logins (UUIDs collision-free in practice), rapid-double-click race on Dismiss (terminal state is the same), per-row import cost of `ZoneInfo`, and several duplicate findings across the three review layers.
